...
package com.project.zhongrenweigong.currency.zxing.camera;
...
import com.project.zhongrenweigong.currency.zxing.android.PreferencesActivity;
...
//		cameraResolution = new Point(1280, 720);
	...
	private void setCameraZoom(Camera camera, int value) {
		Camera.Parameters parameters = camera.getParameters();
		int cur = parameters.getZoom();
		XLog.d("默认 zoom %s", cur);
		if (value <= cur) {
			return;
		}

		int max = parameters.getMaxZoom();
		parameters.setZoom(Math.min(value, max));
		camera.setParameters(parameters);
		cur = parameters.getZoom();
		XLog.d("设置后 zoom %s", cur);
	}
}
