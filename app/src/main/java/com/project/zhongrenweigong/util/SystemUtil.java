...
public class SystemUtil {
    ...
    public static boolean isActivityATop(Context context, String packageName) {
        ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningTaskInfo> list = am.getRunningTasks(100);
        boolean isAppRunning = false;
        //100表示取的最大的任务数，info.topActivity表示当前正在运行的Activity，info.baseActivity表系统后台有此进程在运行
    ...
    //去掉注释是直接跳转三方APP 不注释是跳转系统浏览器
    public static void jumpThirdApp(Activity activity, String scheme) {
        if (activity != null && !activity.isDestroyed() && !activity.isFinishing()) {
//            if (checkUrlScheme(activity, scheme)) {
...
//            }
    ...
    public static void jumpDeepLink(Activity activity, String scheme) {
        if (activity != null && !activity.isDestroyed() && !activity.isFinishing()) {
            Uri uri = Uri.parse(scheme);
            Intent intent = new Intent(Intent.ACTION_VIEW, uri);
    ...
    public static boolean checkUrlScheme(Activity activity, String scheme) {
        Uri uri = Uri.parse(scheme);//scheme
        Intent intent = new Intent(Intent.ACTION_VIEW, uri);
    ...
    public static String changeChinese(int weekday) {
        switch (weekday) {
            case 1:
                return "周一";
            case 2:
...
//    public static void removeActivity(Context mContext, String packageName) {
